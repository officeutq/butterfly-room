了解です。最小差分で、将来の standby 上部 slot 横並び拡張にも乗せやすい形でまとめます。

実装方針

* 上部 `live-overlay-meta-slot` に、live/away 用 `metaPanel` とは別に standby 用アクションパネルを追加
* standby 時だけ「スタンバイをやめて戻る」ボタンを表示
* 既存の `summaryBtn` target をそのまま使って JS の表示制御に乗せる
* offscreen 側に残っている旧導線は撤去
* `finish` 後の notice は「スタンバイを終了しました」に変更
...
* standby で上部 slot にボタンが見える
* live / away では上部 slot は meta 表示に戻る
* ボタン押下で `finish` が呼ばれ、ブースサマリーへ戻る
* スマホ幅でボタンが 44px 以上の押下高さを保つ
* offscreen 側に旧ボタンが残っていない

必要ならこの次に、コミットコメント案と PR 文もそのまま出します。
