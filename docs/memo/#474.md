了解です。
今回は **最小変更で #474 のスコープに合わせる** 方針でいきます。

## 実装方針

* `none / beauty / effect` の3状態にする
* `effect` は内部的に `ActionunitsRabbit.zip` を使う
* UI表示は指定通り **Effect のまま**
* Beauty 調整UIは既存通り **Beauty選択時のみ** 表示
* preload 専用処理は増やさず、既存の `ctx._banubaEffects` キャッシュで対応
...
<div
  ...
  data-ivs-publisher-banuba-extra-effect-url-value="<%= @banuba_extra_effect_url %>"
  data-ivs-publisher-banuba-extra-effect-name-value="<%= @banuba_extra_effect_name %>"
>
  <div id="live_stage">
    <div id="live_media">
      <div
  ...
  <div
    ...
    <div class="live-effect-toggle-group" role="radiogroup" aria-label="加工モード選択">
      ...
      <input
        ...
        class="btn-check"
        name="cast_live_effect_mode"
        id="cast_live_effect_mode_effect"
        value="effect"
        autocomplete="off"
        data-action="change->ivs-publisher#onEffectSelectionChanged">

      <label class="btn btn-outline-light live-effect-toggle-btn" for="cast_live_effect_mode_effect">
        Effect
      </label>
    </div>
  </div>
...
2. 星ボタンで加工モードパネルが開く
3. `なし` で effect が消える
4. `Beauty` で既存の beauty が戻る
5. `Effect` で `ActionunitsRabbit.zip` が反映される
6. `Effect` 選択中は sliders ボタンが消える
7. `Beauty` に戻すと sliders ボタンが再表示され、以前の値がそのまま使える

## もし出やすいエラー

もし `Effect` を選んだ瞬間に失敗するなら、まず次を確認してください。

* `public/banuba/effects/ActionunitsRabbit.zip` が本番コンテナ内にあるか
* ブラウザの Network で `/banuba/effects/ActionunitsRabbit.zip` が 200 か
* Banuba 側がその zip を effect として読める形式か

ここで落ちる場合は、JS実装より **素材zip側の互換性** の可能性もあります。

必要なら次に、エラー内容を見てそのまま潰します。
