...
* `Cast::BoothsController#live` で `Effect.enabled_only.ordered` を取得
* `_effect_panel.html.erb` を DB ベース描画へ変更
* `none` / `beauty` は固定項目のまま維持
* custom effect は radio の `value=effect.key` と `data-effect-zip-filename=effect.zip_filename` を持たせる
* Stimulus 側は `"effect"` 固定をやめて、`none` / `beauty` 以外の任意 key を受け取れるようにする
* Banuba 側は、選択中 radio の `zip_filename` から `/banuba/effects/...` を組み立てて適用
* Effect 一覧UIは横スクロール可能なボタン列へ変更

変更対象は6ファイルです。以下をそのまま置き換えてください。
...
```scss
/* ------------------------------------------------------------
   live beauty / effect panel (#403, #471, #485, #488)
------------------------------------------------------------ */
.live-beauty-overlay,
.live-effect-overlay {
  position: absolute;
...
.live-effect-toggle-group::-webkit-scrollbar {
  height: 6px;
}

.live-effect-toggle-group::-webkit-scrollbar-thumb {
...
* system_admin で `enabled=false` にした effect が配信画面に出ないこと
* `position` 順で並ぶこと
* custom effect を選んだとき、`/banuba/effects/<zip_filename>` が存在して正しく切り替わること
