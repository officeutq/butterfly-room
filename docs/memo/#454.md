確認した結果、**#454 はアプリ本体の認可ロジックはすでに満たしています**。
今回やるべき実装は、**cast が public 視聴導線で viewer token を取得できることをテストで固定する**のが中心です。

## 最終実装方針

1. アプリ本体コードは変更しない

   * `CreateParticipantTokenService` → viewer 時は `StreamSessionPolicy#view_token?`
   * `StreamSessionPolicy#view_token?` → `ViewerPolicy#view_token?` に委譲
   * `ViewerPolicy#viewer?` → `user.present?`
   * `StoreBanChecker` → customer のみ BAN 判定

   この流れで、**cast は viewer token を取得可能**です。

2. integration test を追加する

   * `viewer: cast can get token`
   * これで #454 の完了条件を明文化できます

3. 既存テストとの整合

   * banned customer は引き続き forbidden
   * publisher の cast / admin も従来通り
...
```bash
docker compose exec app bin/rails test test/integration/ivs_participant_tokens_test.rb test/services/authorization/stream_session_policy_booth_casts_test.rb
```

## 期待される完了状態

* cast が `role=viewer` で token を取得できる
* 403 が出ない
* customer / admin / publisher の既存挙動は維持される

この Issue は、実質 **テスト追加で閉じられる内容**です。
