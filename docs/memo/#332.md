了解です。
今回は **「不具合修正」ではなく、Epic #331 に沿った送出仕様変更**として整理して実装します。

## 最終実装方針

### この Issue #332 でやること

* 通常配信開始時の video publish 元を

  * `canvas.captureStream(30)` から
  * `getUserMedia()` の **camera video track 直接 publish**
    に変更する
* Overlay UI は従来どおり **DOM 表示のみ**
* preview のミラーは **video 要素だけ**
* 既存の start / end / error / standby preview-only は壊さない

### この Issue #332 で踏み込まないこと

Epic 全体像に合わせて、今回はここまでは広げません。

* live ↔ away 中の **publish track 本体の差し替え完成**
* away placeholder 送出仕様の完成版
* 解像度上限の最適化
* viewer 側の比率調整

そのため今回のコードは、

* **通常開始時は camera 直 publish**
* canvas は **away のローカル表示や将来拡張のために残す**
  という切り方にしています。
...
* 通常配信開始時の送出 video は **camera track 直 publish**
* `canvas width="1280" height="720"` に引っ張られない
* preview のミラーは見た目だけ
* standby preview-only / start / end / meta reload / mic toggle は維持

## この段階での確認ポイント

確認はこの順がおすすめです。

1. 通常配信開始後、送出が 16:9 固定っぽくならず、端末カメラ比率寄りになっているか
2. preview は引き続きミラー表示か
3. 配信開始 / 配信終了 / エラー表示が壊れていないか
4. standby で preview-only が壊れていないか

なお、**live ↔ away 中の publish track 本体切替**は、この実装ではまだ完成対象にしていません。
そこは Epic #331 の後続 Issue で切る前提に揃えています。
