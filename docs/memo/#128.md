了解。#128 は **`ivs_publisher_controller.js` だけで直す**方針でいきます（HTML側は現状のまま）。目的は「standby connect 時に video-only プレビューを起動し、配信開始で video+audio に取り直して publish」を追加しつつ、既存の #78 以降の追加機能（resume / before-cache / away / cleanup / UI）を壊さないことです。

---

## 最終実装計画（順序つき）

1. `connect()` に **standby preview-only 起動**を追加

   * 条件：`initialBoothStatusValue === "standby"` かつ `tokenUrlValue` が存在（= stream_session がある）
   * 動作：`getUserMedia({ video, audio:false })` → `previewTarget.srcObject` → `play()`
   * 注意：Stage join / publish / canvas loop は **絶対に開始しない**

2. `startBroadcast()` を **preview-only → broadcasting** に安全に移行

   * もし preview-only が動いていたら、古い stream の track を stop して解放
   * `getUserMedia({ video, audio:true })` を取り直す（Issue本文どおり）
   * 以降は現行ロジックを維持（token→canvas loop→publish→patch live→away適用）

3. cleanup を強化

   * 既存 `_cleanupMediaAndCanvas()` は `_media` の tracks を stop するのでOK
   * ただし preview-only 中は `_media` に入るので、そのまま stop 対象に入って安全

4. 手動テスト（Done条件対応）

   * standby でプレビュー表示（黒でない、ミラーあり、音声なし）
   * standby のまま viewer 参加不可（token 409 not_joinable）
   * 配信開始で音声付きになり viewer に映像/音声が届く
   * away 切替・音声トグル・終了が既存どおり

---

## 貼り替え用コード（`app/javascript/controllers/ivs_publisher_controller.js` 全文）
...
1. cast でスタンバイ開始 → `cast/booths/live` に遷移

   * すぐに video が黒じゃなくなり、**ミラーされたプレビュー**が見える
   * マイク権限は要求されない（OS/ブラウザの許可UIが出ない、またはカメラのみ）

2. standby のまま viewer 側で視聴を試す

   * token が 409 `not_joinable` になり join できない（既存どおり）

3. cast 側で「配信開始」押下

   * マイク許可が出る（初回のみ）
   * viewer に映像＋音声が届く
   * booth.status が live になる（UIの状態も live 側になる）

4. 離席 → 復帰 → 配信終了

   * away で canvas 側表示（既存どおり）
   * 離席中音声トグルが効く（既存どおり）
   * 配信終了で cleanup され、サマリーに寄る（既存どおり）

---

## コミットメッセージ案 / PR本文（必要ならすぐ出します）

実装が終わって動作確認できたら、「コミットメッセージ案 / PR本文」もこのままの文脈で出せます。
