...
了解です。
復帰フラグを廃止して、**「既存自分配信への入場時は常に自動復帰を試す」** 形のコピペ実装を出します。
...
先頭の data 属性に `auto_resume_on_entry` を追加してください。
それ以外はそのままで大丈夫です。
...
<div
  ...
  data-ivs-publisher-auto-resume-on-entry-value="<%= @auto_resume_publish %>"
...
* `sessionStorage` ベースの復帰フラグを廃止
* 自分が started_by_cast_user の current session で `live / away` のときだけ
  **入場時に毎回自動復帰を試す**
* `away` で入っても復帰時は `normal/live` に正規化
* 失敗したら既存の「配信に戻る」ボタンへフォールバック

---

## 確認ポイント

* 配信中にホームへ行って戻る → 自動復帰する
* 配信中にリロード → 自動復帰する
* 別タブで同じ自分配信を開く → 自動復帰を試す
* 自動復帰失敗時 → エラーメッセージ表示＋「配信に戻る」で再試行できる
* `standby` では従来通り preview only

必要なら次に、この変更に合わせた**コミットメッセージとPR文の更新版**も出します。
