了解です。
以下、**metaPanel を JS 表示切替に寄せた修正版**のコピペ用一式です。
...
### 2. `app/views/cast/booths/_stream_meta_display.html.erb`（新規）

```erb
<%# locals: booth:, stream_session: %>
<% title_display = stream_session.title.presence || booth.name %>

<div class="live-meta-panel-header">配信タイトル / 説明</div>

<div class="live-meta-panel-body">
  <div class="live-meta-block">
    <div class="live-meta-label">タイトル</div>
    <div class="live-meta-title"><%= title_display %></div>
  </div>

  <% if stream_session.description.present? %>
    <div class="live-meta-block">
      <div class="live-meta-label">説明</div>
      <div class="live-meta-description"><%= simple_format(stream_session.description) %></div>
    </div>
  <% end %>
</div>
```
...
`live drink panel (#321)` の前あたりに、これを追加してください。
...
* 初期 standby では meta が見えない
* standby のまま画面遷移なしで配信開始すると meta が表示される
* live → away でも表示されたまま
* 配信終了後はリロードで消える
* 長い説明は meta slot 内だけスクロールする
* debug 表示中でも panel 自体は普通に見える

気になる点が 1 つあります。
今の `live.html.erb` だと `live-slot-label` は常に出る構造なので、debug ON のときは **meta panel と “meta slot” ラベルが両方見える** 可能性があります。実際に邪魔なら、そのラベルだけ standby 時だけ出す形に次で微修正できます。
