...
今回の修正点は `app/javascript/controllers/ivs_publisher_controller.js` に絞ります。

### 1. 切替中フラグを追加

`live→away` / `away→live` の連打や、Turbo 応答競合を防ぐために、`_switchingVideoSource` を追加します。

### 2. `onBoothStatusPatched` を非同期で安全化

今は PATCH 成功後に即 `_mode` を更新して `_applyCurrentMode()` していますが、ここを

* 目標状態を決める
* 目標 source を決める
* 実際の publish 切替を試す
* 成功したら `_boothStatus` / `_mode` を確定
* 失敗したら内部状態を元に戻してエラー表示

という順に変えます。

### 3. publish 切替処理を「成功/失敗が返る関数」にする

今の `_switchPublishedVideoSource()` は内部で `refreshStrategy()` を呼ぶだけなので、
これを **例外を投げて失敗検知できる形** に整理します。

### 4. `_refreshStageStrategy()` を失敗時に throw する

今は `_setError()` だけで終わっていますが、切替側で失敗扱いできるようにします。

### 5. 失敗時は最後の正常な publish source を維持

A案なのでサーバロールバックはしません。
ただしクライアント内部では、

* `_currentVideoStageStream`
* `_publishedVideoTrack`
* `_publishedVideoSource`

を正常値へ戻します。

### 6. UI 連打抑止

切替中は camera on/off ボタンを無効化します。

---

## paste-ready code

差し替え対象は `app/javascript/controllers/ivs_publisher_controller.js` 全文です。
...
1. live → away
   `booth.status=away` になり、viewer に placeholder が届く

2. away → live
   `booth.status=live` になり、viewer に camera が戻る

3. 切替連打
   ボタン連打しても二重切替にならない

4. 失敗ケース
   `refreshStrategy()` が失敗したとき、同じ操作を連続実行せず、エラー表示だけで止まる

5. mic
   away/live 切替で mic の論理状態が勝手に変わらない

必要なら次に、この Issue #335 のコミットコメントと PR 文まで続けて整えます。
