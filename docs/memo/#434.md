了解です。
この Issue #434 は、**standby 中の session 終了導線だけを取り除き、既存の standby session 再利用フローはそのまま維持**する実装で閉じるのが最も自然です。

## 実装方針

* `live.html.erb` の standby パネルから **「スタンバイをやめて戻る」** を削除
* `finish` は **配信終了時のみ** `endBroadcast()` から使う
* `summaryPanel` は standby 中の補助 UI として残し、**配信設定のみ**を置く
* JS 側の `summaryBtn` 依存を削除して、不要な target / 表示制御を整理
* `show` / `create` / `finish` / `EndService` などのサーバ側フローは今回は変更しない

これで、

* offline → standby 開始
* standby 中に離脱して戻る → 既存 session 再利用
* standby 中に手動終了して summary へ戻る → 廃止
* 配信終了 → finish → result
  という構造になります。
...
* standby 画面で「スタンバイをやめて戻る」が消えている
* standby では「配信設定」は開ける
* standby のまま画面離脱→再入場で同じ session が使われる
* 配信開始は従来どおり動く
* 配信終了は footer の「配信終了」から result へ遷移する
