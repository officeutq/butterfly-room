...
* 起動時に固定で Beauty を当てるのをやめる
* `this._selectedEffect` を唯一の正とする
* `none` は `player.clearEffect()`
* `beauty` は `beauty_base.zip` を `player.applyEffect(...)`
* Beauty effect 適用後に既存の Beauty 値を `reloadConfig` で再反映
* 未対応の `effect_heart` / `effect_neon` は今回のスコープ外なので UI 上で無効化

変更ファイルは3つです。

1. `app/javascript/controllers/ivs_publisher_controller.js`
2. `app/javascript/controllers/ivs_publisher/banuba_session.js`
3. `app/views/cast/booths/_effect_panel.html.erb`
...
export default class extends Controller {
  ...
  _isSelectableEffect(value) {
    return ["none", "beauty"].includes(value)
  }
...
* Standby で preview が出ている状態で `Beauty` / `なし` を切り替える
* `なし` で effect が外れる
* `Beauty` で `beauty_base.zip` が再適用される
* `Beauty` 選択時だけ sliders ボタンが出る
* sliders で既存の美白・輪郭・目・鼻・口がそのまま動く
* `なし` のときに Beauty スライダーを触れない
* away → live の切替後も effect 状態が維持される

必要なら次に、`docker compose` 前提の確認コマンドとコミットメッセージ案までまとめます。
