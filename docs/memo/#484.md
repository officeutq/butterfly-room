はい、その理解で実装します。
**`config.json` は初期読み込み時だけ使い、以後のエフェクト切替や「なし」からの復帰では保持中の `_beautyState` を使う** 方針です。
...
`errors.js` と `api_client.js` は今回は変更不要です。
...
* `public/banuba/effects/beauty_base/config.json` は実ファイルを配置してください
* 今回の実装では、初回読込後は `_beautyState` を使い続けます
* `beauty -> none -> beauty` や `beauty -> effect -> beauty` でも、調整値は保持されます
* `config.json` に存在しない `nose` / `lips` は 0 起点で補完されます
* `reloadConfig` のたびに元JSON全体を土台にしているので、UI未対応項目は落ちません

必要なら次に、コミットメッセージと PR 文も整えます。
