...
* `lastManualMicEnabled` を新設し、**最後にユーザーが手動で選んだ mic 状態**を保持する
* 初期値は `true` にして、**一度も手動操作していない場合は ON 扱い**
* `toggleMic()` では

  * `lastManualMicEnabled` を更新
  * 実際の mic 状態にも反映
  * UI更新
* `live → away` では

  * **その瞬間だけ** 安全のため強制 OFF
  * ただし `lastManualMicEnabled` は変えない
* `away中` は
...
* `away → live` では
  ...
  * `lastManualMicEnabled` の値へ復元

今回の仕様だと、`away中も実音声は手動操作に従う` ので、
「away 中は常時強制OFF」にはしません。
強制OFFは **away 突入時の一回だけ** です。
...
1. live で mic ON → away にすると、即 OFF 表示になり実音声も切れる
2. away 中に mic を ON にすると、アイコンも ON になり実音声も ON になる
3. away 中に mic を OFF にすると、アイコンも OFF になり実音声も OFF になる
4. away → live 復帰で、最後に手動で選んだ状態が維持される
5. 一度も mic を触らずに away → live した場合、live 復帰後は ON になる

必要なら次に、この Issue #372 のコミットコメントと PR 文をそのまま使える形でまとめます。
