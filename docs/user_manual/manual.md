...
- 「プロフィール情報」: 自分のプロフィール詳細を確認し、「プロフィールを編集」から表示名、自己紹介、プロフィール画像を編集できます。
- 「プロフィール情報」カードの「電話番号認証済み／電話番号未認証」バッジで認証状態を確認できます。認証はプロフィール編集のアカウント欄から行います。

## プロフィールを編集する

1. dashboard（ダッシュボード）で「プロフィール情報」を開き、本人の詳細画面で「プロフィールを編集」を押します。
...
保存が完了すると自分のプロフィール詳細へ移動し、「プロフィールを更新しました」と表示されます。詳細画面の名前の下にある、鉛筆アイコン付きの「プロフィールを編集」から再編集できます。変更がない場合も「保存」で編集を完了できます。

※以下の保存後画像は旧導線です。現在はホームではなく本人のプロフィール詳細へ移動します。

![旧導線のプロフィール更新後](images/customer/profile/03_after_update_home.png)

TODO: プロフィール画像アップロードの具体的な操作と推奨画像サイズは、別途追記します。

## メールアドレスを変更する

1. プロフィール編集の「アカウント」で「メールアドレスを変更」を押します。
2. モーダル内で新しいメールアドレスと現在のパスワードを入力し、「変更する」を押します。
3. 成功するとモーダルが閉じ、アカウント欄のメールアドレスが更新されます。

メールアドレス・電話番号は公開プロフィールに表示されません。各変更はモーダルで確定し、プロフィールの「保存」は不要です。編集中の表示名・自己紹介・画像はそのまま残り、プロフィールの未保存変更を破棄しても確定済みのアカウント変更は取り消されません。

## 電話番号を認証する

1. dashboard（ダッシュボード）で「プロフィール情報」を開き、本人の詳細画面で「プロフィールを編集」を押します。
2. 「アカウント」の認証状態を確認し、「電話番号を認証する」または「電話番号を変更」を押します。
3. モーダル内で認証する電話番号を入力します。
4. 「認証コードを送信」を押します。
5. SMS（ショートメッセージ）で届いた6桁の認証コードを入力し、「認証して登録」を押します。
6. 成功するとモーダルが閉じ、同じ画面の電話番号と認証状態が更新されます。

認証途中で閉じても登録番号は変わりません。再度開くとコード入力を再開できます。「電話番号を入力し直す」で対象番号を変更できます。コードの有効期限は5分、再送は前回送信から60秒後です。

※以下の電話番号認証の画像は旧画面です。現在はプロフィール編集内のモーダルで操作します。
...
※2026-09-18：店舗情報・編集導線の本文を更新しました。ダッシュボード・店舗・口座の画像は旧仕様です。今回のブラウザー確認は専用テストDBで行い、全体の撮影用データを更新しないためマニュアル画像の一括再撮影は未実施です。
...
主なカードは、店舗を選択、ブース管理、店舗情報、キャスト一覧、店舗管理者招待、配信者別数値一覧、通報一覧、精算（予定・履歴）です。

「店舗を選択」は複数の管理店舗があり、本人が配信中・離席中ではない場合に表示されます。1店舗の場合は自動設定され、店舗情報へ直接進めます。
...
店舗所属者情報の「所属一覧」では、選択店舗のキャストと店舗管理者を区分して確認できます。キャストには所属ブースと所属解除ボタンがあり、管理者には所属解除ボタンはありません。
...
ダッシュボードの「店舗所属者情報」を開き、「キャスト招待」タブを選びます。有効・使用済み・期限切れの招待を新しい順に表示します。確認できる項目は状態、管理者用メモ、発行日時、有効期限、発行者、承認者です。

一覧上部の「キャスト招待URLを発行」でキャスト招待モーダルを開けます。招待カード自体は閲覧専用で、URL・再共有・コピー・取消・メモ編集の操作はありません。取消済みは表示しませんが、履歴データは保持されます。旧キャスト招待画面のURLはこのタブへ転送されます。

## 店舗管理者を招待する

ダッシュボードの「店舗所属者情報」を開き、「店舗管理者招待」タブから追加の店舗管理者を招待できます。旧管理者招待一覧のURLもこのタブへ転送されます。

1. 「店舗管理者招待URLを発行」を押すとモーダルが開き、URLが発行されます。
2. 店舗名・期限を確認し、必要なら管理者用メモを入力します。メモは招待相手には表示されません。
3. 「招待URLを共有」または「招待URLをコピー」で本人へ送り、「閉じる」を押します。修正したメモも保存し、一覧へ反映されます。
4. 共有・コピー前のキャンセルはURLを無効化します。端末の共有画面の中止だけでは取り消しません。保存・取消失敗時はモーダル内で再試行できます。

管理者招待一覧は有効・使用済み・期限切れを表示し、取消済みを除外します。カードはキャスト招待と同じ6項目の閲覧専用で、URL・再共有・コピー・取消・メモ編集の操作はありません。共有・コピーは発行モーダル内で行います。権限がある場合の「店舗責任者を登録（代行用）」もこのタブにあります。管理者招待はキャスト招待用の初回案内を進めません。
