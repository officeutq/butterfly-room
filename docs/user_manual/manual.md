...
ダッシュボードの「店舗所属者情報」を開き、「キャスト招待一覧」タブを選びます。有効・使用済み・期限切れの招待を新しい順に表示します。確認できる項目は状態、管理者用メモ、発行日時、有効期限、発行者、承認者です。

一覧上部の「招待URLを発行」でキャスト招待モーダルを開けます。招待カード自体は閲覧専用で、URL・再共有・コピー・取消・メモ編集の操作はありません。取消済みは表示しませんが、履歴データは保持されます。旧キャスト招待画面のURLはこのタブへ転送されます。

## 店舗管理者を招待する

「店舗所属者情報」の「管理者招待一覧」から追加の店舗管理者を招待できます。従来の「店舗管理者招待」カード・URLもこのタブへ進みます。

![店舗管理者招待画面](images/store_admin/invitations/04_store_admin_invitation_index.png)

1. 「招待URLを発行」を押すとモーダルが開き、URLが発行されます。
...
管理者招待一覧は有効・使用済み・期限切れを表示し、取消済みを除外します。既存のURL表示・共有・コピーは維持し、共有完了の記録だけが失敗した場合は記録の再試行ができます。権限がある場合の「店舗責任者を登録（代行用）」もこのタブにあります。管理者招待はキャスト招待用の初回案内を進めません。
