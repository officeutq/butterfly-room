...
※文書整備上の残作業：管理用店舗情報への集約は#1251〜#1253で実装済みです。旧ブース管理の撤去・新規作成カード・閉鎖済み情報画面の操作整理は#1291〜#1294で対応します。後続画面の完成を前提にした手順・画像は掲載しません。
...
現在は旧ブース管理画面が残っており、一覧確認、新規作成、ブース編集などを行えます。操作対象の変更にはヘッダーを使用してください。旧一覧の切替ボタンの撤去と「ブース新規作成」カードへの分離は後続対応です。
