...
※2026-09-18の店舗情報・編集導線を本文に反映しました。店舗情報・編集・口座・ダッシュボードの既存画像は旧仕様です。今回の確認は別の専用テストDBで行い、マニュアル全体の撮影用データを更新しないため一括再撮影は未実施です。
...
![store_admin dashboard](images/store_admin/selection/01_dashboard.png)
...
※ブース情報への統合と新規作成カードは#1291〜#1293で実装済みです。操作は下記と共通操作手順を参照してください。既存のダッシュボード画像は改修前のもので、全体の画像再取得・横断確認は#1294で扱います。
