...
2026-09-18追記：ブース管理の現行手順は[ブース情報・新規作成・閉鎖](booth_management_manual.md)、撮影は[管理統合の撮影記録](booth_management_capture.md)を参照してください。旧管理一覧GETは廃止され、管理対象の選択は共通モーダル、個別管理はブース情報、新規作成は独立カードです。以下の一覧画面を含む調査・撮影候補は当時の記録として保持します。
...
- 利用できる主な画面:
  - `/` ホーム
  - `/dashboard` ダッシュボード
  - `/users/:id` プロフィール詳細（「プロフィール情報」カードは本人の詳細へ移動）
  - `/profile/edit` プロフィール編集
  - `/phone_verification` 電話番号認証（プロフィール編集のアカウント欄からモーダルで操作）
  - `/favorites/...` お気に入り
  - `/stores/:id` 店舗詳細
  - `/booths/:id` ブース詳細
...
| 条件 | 表示される主なカード |
| --- | --- |
| `at_least?(:customer)` | プロフィール情報（本人の詳細へ移動、電話番号認証状態のバッジ付き） |
| `cast? && @cast_booths_count >= 2` | ブース一覧 |
| `at_least?(:cast)` | ブース情報（情報画面から編集・配信履歴・共有へ進む） |
