2026-09-18：ブース管理統合後の撮影は[専用テストDBでの新しい手順](booth_management_capture.md)を参照してください。以下の旧一覧を使う手順・画像一覧は過去の記録として保持します。

※2026-09-18：#1252・#1253に合わせ撮影スクリプトを店舗情報経由へ更新しました。以下は以前の撮影記録です。既存画像は未更新です。今回のブラウザー検証は専用テストDBで実施し、全役割のマニュアル撮影データを更新しないため一括再撮影していません。

# 第4回 store_admin 通常操作画面撮影記録

## 2026-09-16の選択改修

現在の撮影方法・結果は[ブース・店舗選択の撮影記録](selection_capture.md)を参照してください。専用テストDB、testサーバーの3102番ポートで、ヘッダーの選択モーダル・情報・編集フォーム・配信履歴を撮影しました。新画像は各役割の`selection/`配下に保存しています。旧ブース一覧や編集URLによる自動切替を今回の期待結果として使いません。

以下は過去の撮影記録です。コマンド・遷移・外部通信の説明は当時の実装に対応します。現在は実配信者の制御により、準備入口でもIVS照会が必要になる場合があります。疑似Stageのまま実配信の撮影へ進まないでください。今回再撮影していない画像・理由は上記の共通記録にまとめています。
