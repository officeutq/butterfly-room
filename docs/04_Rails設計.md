...
- 情報・編集・履歴は認可後に選択との一致を照合する。履歴URLでも選択は上書きしない。キャストの閉鎖済み履歴の直接閲覧は既存権限を維持し、選択とは分離する。古いフォームの保存対象を新しい選択先へ読み替えず、409で拒否する。
...
- 情報画面の切断状態は対象ブースだけを取得する（`publisher_disconnect_state?scope=booth`）。通常の準備・開始で使用する「対象ブースまたは操作本人」の状態・開始制限は維持する。初期HTML・通知・状態GETは保存済み結果だけを表示し、切断の実行や回数の変更は行わない。管理者の閉鎖後は選択を維持し、共有・編集を隠して履歴を表示する。閉鎖済みの直接閲覧経路の整理は#1292、旧一覧撤去は#1293に残す。
