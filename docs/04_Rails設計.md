...
#1224の追加試作では、未設定・未編集の店舗画像だけ、同じAI検索で確認した公式サイト・公式SNSの代表画像を仮反映する。取得順は公式サイト、X、Instagram、TikTok、YouTube。店舗・利用者に結び付いた短時間有効のトークンと初回設定の認可を使い、`ImageImportService`が画像取得、`PublicImageFetcher`が接続先・転送・容量・時間の制限を担当する。取得時にDBや添付を作成せず、既存の画像組保存へ接続する。通常編集は画像取得を行わない。詳細は`docs/design/store_ai_autofill.md`の26節を参照。
