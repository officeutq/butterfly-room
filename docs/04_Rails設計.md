...
### ブース情報・編集・履歴の導線（#1248 / #1274）

#1274で、ログイン・通常表示・対象照合・選択POSTを`CurrentSelection`へ接続した。session書き込みは成功結果の保存へ集約する。URLの対象への上書きと最小ID補完は削除した。詳しくは[入口対応表](design/current_selection_entries.md)。以下の#1248段落は改修前の経緯として保持し、現行の接続範囲は対応表を優先する。ステージング・本番には未反映。

以下は現行の#1248実装。#1271の[選択の共通契約](design/current_selection.md)では、後続#1273・#1274でsession保存と共通判定を接続し、URL・最小IDによる選択上書きを除去する。①②の固定・③の維持に対し、店舗変更後の未設定維持と必要操作時の自動設定（D01）、キャスト招待承認成功時の切替と通知（D02）を合意した例外とする。判定はService、session保存・応答はControllerの共通接続箇所に集約する。選択のためのDBテーブルや別の実配信者記録を作らない。古いフォームは対象を別店舗へ読み替えず、不一致を409として入力保持・再選択を案内する。#1271自体でアプリの動作を変更しない。
