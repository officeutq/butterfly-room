...
- 情報画面の切断状態は対象ブースだけを取得する（`publisher_disconnect_state?scope=booth`）。通常の準備・開始で使用する「対象ブースまたは操作本人」の状態・開始制限は維持する。初期HTML・通知・状態GETは保存済み結果だけを表示し、切断の実行や回数の変更は行わない。管理者の閉鎖後は選択を維持し、共有・編集を隠して履歴を表示する。#1292では閉鎖済み情報と履歴0件の表示・状態・認可を確認し、`Cast::StreamSessionsController#share`も閉鎖済みは404にする。旧一覧撤去は#1293に残す。
