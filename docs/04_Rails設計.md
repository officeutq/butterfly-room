...
2026-09-16のローカル実装。ステージング・本番には未反映。[選択の共通契約](design/current_selection.md)と[41項目の入口対応表](design/current_selection_entries.md)を参照する。
...
- 情報・編集・履歴は認可後に選択との一致を照合する。履歴URLでも選択は上書きしない。#1292ではキャストの担当する閉鎖済み情報・履歴を選択とは分離して閲覧できるようにする。候補0件や別の有効な選択があっても、履歴から同じ閉鎖済み情報へ戻れる。閉鎖済みの選択・編集・共有・準備・開始は許可しない。古いフォームの保存対象を新しい選択先へ読み替えず、409で拒否する。
