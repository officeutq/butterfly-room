...
初回・通常編集画面は画像の有無にかかわらず既存のJSON保存応答を使用し、保存失敗でも入力値・バッジ・情報源を同じ画面に保持する。AI状態を店舗属性やDBカラムには追加しない。通常編集は詳細フォームを最初から表示し、初回と同じ空欄・バッジ付き項目への直接反映を使う。保存済み非空欄はバッジなしで保護する。共通AI検索と画像取得は別々に利用者ごと10分間10回とし、それぞれ全店舗・全導線で合算する。通常保存の`Stores::UpdateService`とダッシュボード遷移、初回公開の`Stores::CompleteRegistrationSetup`と計測を分ける。詳細は `design/store_ai_autofill.md` の25・27節を参照する。
