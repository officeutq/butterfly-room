...
#1224の追加試作では、未設定・未編集の店舗画像だけ、同じAI検索でURL入力候補に採用できたページの代表画像を仮反映する。画像だけに追加の公式根拠やSNSの認証バッジは要求しない。取得順は公式サイト、X、Instagram、TikTok、YouTube。店舗・利用者に結び付いた短時間有効のトークンと初回設定の認可を使い、`ImageImportService`が画像取得、`PublicImageFetcher`が接続先・転送・容量・時間の制限を担当する。取得時にDBや添付を作成せず、既存の画像組保存へ接続する。通常編集は画像取得を行わない。詳細は`docs/design/store_ai_autofill.md`の26節を参照。

初回画面は画像の有無にかかわらず既存のJSON保存応答を使用し、保存失敗でも入力値・バッジ・情報源を同じ画面に保持する。AI状態を店舗属性やDBカラムには追加しない。通常編集の画面・選択反映は維持するが、共通AI検索の回数制限はユーザーごとに10分間10回に変更する。詳細は `design/store_ai_autofill.md` の25節を参照する。
