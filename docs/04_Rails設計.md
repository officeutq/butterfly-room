...
* `GET /admin/stores/:id` は公開店舗詳細とは別の、店舗管理者・システム管理者向けの情報画面。`Admin::StoresController#show` はURL対象の存在・管理者所属を確認し、既存の `require_selected_store!` で選択と照合する。権限外は403、不存在は404、別店舗が選択中なら409でヘッダー選択を案内し、別店舗の内容へ代替しない。
* 公開・非公開とも管理可能な店舗を表示する。店舗画像・基本情報は公開詳細と同じ部分テンプレート、ドリンクは `with_attached_custom_icon.ordered` で有効／無効を含め、口座は `active_payout_account` の現在設定のみを取得する。
* 振込先は既存編集画面と同じ表示用部分テンプレートを使い、銀行口座番号・ゆうちょ番号は末尾4桁だけの伏字表示とする。口座の全文や無効な過去口座をHTMLへ埋め込まない。
* 表示は読み取り専用で、店舗・ドリンク・口座・配信・初回案内の保存処理を呼ばない。共通選択の自動設定・固定は維持する。各編集とヘッダー切替後の戻り先は#1252、ダッシュボード・チュートリアルの入口は#1253で接続した。通常編集は対象の管理用店舗情報へ戻り、公開店舗詳細の編集リンク・専用復帰は廃止した。ドリンク・口座のGETは明示された `selection_store_id` を既存の `require_form_store!` で認可・選択照合し、保存時にも元の対象を維持する。固定の戻り先指定・代行登録の例外は[導線設計3.1節](07_モード導線設計.md)を参照。
...
### ブース・店舗選択と情報・編集・履歴（#1273〜#1276）

2026-09-16に実装済み。環境反映・実機確認は[ステージング記録](ops/current_selection_staging_verification.md)を参照。[選択の共通契約](design/current_selection.md)と[41項目の入口対応表](design/current_selection_entries.md)を参照する。

- 共通判定は `CurrentSelectionService`、ログイン・各入口での呼び出し、成功結果のsession保存と応答は `CurrentSelection` に集約する。URL・最小IDによる有効な選択の上書きは行わない。D01（店舗変更後）とD02（招待承認成功後）は共通契約の例外に従う。
- 本人の固定は `StreamSession.current_broadcast_for_selection` のDB記録で判定する。準備作成者・担当者・未確定の開始権を代用しない。不整合は未配信と見なさず案内し、通常の選択判定ではIVSへ通信しない。
- 情報・編集・履歴は認可後に選択との一致を照合する。履歴URLでも選択は上書きしない。#1292ではキャストの担当する閉鎖済み情報・履歴を選択とは分離して閲覧できるようにした。候補0件や別の有効な選択があっても、履歴から同じ閉鎖済み情報へ戻れる。閉鎖済みの選択・編集・共有・準備・開始は許可しない。古いフォームの保存対象を新しい選択先へ読み替えず、409で拒否する。
- `selection_switch_controller.js` は未保存確認後に選択POSTを行い、成功した戻り先へ移る。必要なら実際の配信準備インスタンスへ初回開始要求の取消を依頼する。キャンセル・失敗時は入力・選択・画面を保持する。
- 候補表示GETや情報確認用の選択では準備を作らない。配信準備への明示的な移動だけ `Booths::PrepareSelectedBoothService` が既存の配信制御へ委譲する。他者配信中・閉鎖済み・非公開店舗は情報画面で理由を案内する。
- `selection_lock_controller.js` は配信成功・本人配信の回復時にヘッダーを名前だけの表示へ変える。古いタブからのモーダル要求も最新DBで拒否し、`shared/selection_locked` で画面と配信接続を保持する。
- 通常のブース編集の保存成功後は対象の情報へ戻る。招待直後の初回編集はホーム遷移を維持する。
- #1291：管理者向けの強制終了・閉鎖と切断結果をブース情報へ接続する。`Admin::BoothsController`はURL対象を管理可能な店舗全体から解決し、共通選択と照合してから既存の`ForceEndService`／`Booths::ArchiveService`を呼ぶ。表示時の配信ID・世代を送信し、対象不一致では409、成功・業務エラーのHTML応答は同じブース情報へ戻す。個別配信セッションの通常終了・取消・消化APIの選択条件は変更しない。
- 情報画面の切断状態は対象ブースだけを取得する（`publisher_disconnect_state?scope=booth`）。通常の準備・開始で使用する「対象ブースまたは操作本人」の状態・開始制限は維持する。初期HTML・通知・状態GETは保存済み結果だけを表示し、切断の実行や回数の変更は行わない。管理者の閉鎖後は選択を維持し、共有・編集を隠して履歴を表示する。#1292では閉鎖済み情報と履歴0件の表示・状態・認可を確認し、`Cast::StreamSessionsController#share`も閉鎖済みは404にする。#1293で旧管理一覧を撤去し、`Admin::BoothsController#index`は店舗選択を要求せず認証・役割確認後にダッシュボードへ戻す。作成の店舗確認はnew/createに限定して維持し、ダッシュボードの「ブース新規作成」から進む。店舗未選択ならカードから共通店舗モーダルを開き、選択後に作成へ戻る。旧一覧への戻り先もダッシュボードへ読み替える。横断した試験と実機未確認範囲は[管理統合の検証記録](design/booth_management_validation.md)、操作は[管理手順](user_manual/booth_management_manual.md)を参照する。
...
* 他に有効な管理者がいる店舗・ブース・未開始準備は維持する。新しい実配信者記録の経路では、本人配信を終了・返却し、初回開始が未確定なら本人接続だけ取り消す。準備作成者という理由では終了しない。唯一管理者店舗は非公開化してブース・キャスト所属・未使用招待を整理する
* 新経路の退会はUserと管理店舗を`FOR NO KEY UPDATE`で直列化し、ブースをID順に取得して開始状態を確認する。開始側の人物・店舗外部キー参照と逆順に待たず、開始・確定側はブース取得後に退会済みかを再照合する
...
* 認証済みactorと注文IDをServiceへ渡し、結果／認可エラーを返す
  （本人判定・現在配信状態・FIFOはService側で担保。#1280共通有効化後は実配信者本人だけを許可し、管理者の代理消化は許可しない）
...
#1284：共通有効化後はブース→セッション→注文の順にロックし、実配信者本人・現在のlive/away・既存配信権限を確認する。消化確定と`ConsumptionCommentService`による通知保存を同じトランザクションで実行する。最外側commit後に`NotifyDrinkConsumptionJob`がキュー・財布・コメント表示を通知し、失敗は同じコメントIDだけで再試行する。5回失敗したジョブは通常の失敗ジョブとして再実行でき、確定済み消化のAPI再送は従来どおり409とする。通常コメントの投稿条件や過去コメントのuser_idは変更しない。
...
初回設定の入口は店舗名の確認とAI入力に限定し、検索後に同一画面内で詳細フォームへ進む。初回・再検索とも候補モーダルを使わず、空欄またはAI・未バッジ付きの項目だけを反映する。利用者の編集でバッジを消し、編集済み非空欄・店舗名・画像をAIで上書きしない。検索失敗でも手入力で続行でき、検索・画面切り替えだけでは保存・公開・登録成果計測を行わない。

#1224の追加試作では、未設定・未編集、または利用者が明示的に削除した店舗画像だけ、同じAI検索でURL入力候補に採用できたページの代表画像を仮反映する。画像だけに追加の公式根拠やSNSの認証バッジは要求しない。取得順は公式サイト、X、Instagram、TikTok、YouTube。店舗・利用者に結び付いた短時間有効のトークンと店舗編集権限を使い、`ImageImportService`が画像取得、`PublicImageFetcher`が接続先・転送・容量・時間の制限を担当する。取得時にDBや添付を作成せず、既存の画像組保存へ接続する。#1228で代行・通常編集も共通の`StoreAiAutofillsController#image`を利用する。画像取得だけは初回pendingを要求しない。詳細は`docs/design/store_ai_autofill.md`の26・27節を参照。

初回・通常編集画面は画像の有無にかかわらず既存のJSON保存応答を使用し、保存失敗でも入力値・バッジ・情報源を同じ画面に保持する。AI状態を店舗属性やDBカラムには追加しない。通常編集は詳細フォームを最初から表示し、初回と同じ空欄・バッジ付き項目への直接反映を使う。保存済み非空欄はバッジなしで保護する。共通AI検索と画像取得は別々に利用者ごと10分間10回とし、それぞれ全店舗・全導線で合算する。通常保存の`Stores::UpdateService`と初回公開の`Stores::CompleteRegistrationSetup`・計測を分ける。通常保存後は原則ダッシュボードへ遷移する。#1234で公開店舗詳細からの編集入口を追加し、この入口に限り公開中なら同じ店舗の詳細へ戻す（非公開ならダッシュボード）。詳細は `design/store_ai_autofill.md` の25・27節と `07_モード導線設計.md` の3.1節を参照する。
