...
以下は既存の一覧・作成・アーカイブ仕様。#1255の共通選択・ヘッダー・準備切替はローカル実装済みで、キャストの旧一覧GETはダッシュボードへ戻す。管理者は閉鎖済みを候補数へ含め、閉鎖後も選択を維持する。#1290の後続では管理者の旧一覧を撤去し、新規作成を独立カード、管理操作をブース情報へ移す。閉鎖済みは履歴確認用とし、共有非表示・履歴ボタン表示は#1292の未実装範囲。準備・開始・編集は拒否する。詳細は[選択の共通契約](design/current_selection.md)、[利用者確認と未確認範囲](ops/current_selection_local_verification.md)を参照。今回の選択改修はステージング・本番には未反映。
