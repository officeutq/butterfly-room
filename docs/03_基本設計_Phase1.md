...
- 公開ページはページ単位の title、description、canonical、OGPを設定する。`/` の既存の非表示H1は公式タグラインを示し、サイトを表す `WebSite` 構造化データはドメインルートの `/` だけに配置する。店舗詳細のdescriptionとOGP descriptionは、登録済みの店舗名・エリア・業態の日本語表示名・営業時間・空白を正規化した店舗説明から同じ文章を160文字以内で生成し、住所全文は含めない。画面表示と既存導線は変更せず、sitemapには公開店舗だけを含める
