...
以下は既存の一覧・作成・アーカイブ仕様。#1255・#1290の改修では、管理対象の一覧を共通の選択モーダルへ集約し、ブース新規作成は独立カード、管理操作は管理用ブース情報へ移す。管理者は閉鎖済みを候補数へ含め、閉鎖後も情報・履歴を確認できる。閉鎖済みの編集・共有・準備・配信開始は認めない。詳細と未決事項は[選択の共通契約](design/current_selection.md)。設計段階でアプリへの適用は未実施。
