...
* [x] `users`（Devise: email+password）
* （必要なら）`profiles`（cast/customerの拡張。Phase1はusersに寄せてもOK）

#### 店舗・所属

* [x] `stores`
* [x] `store_memberships`（キャスト/管理者の所属）
...
* [x] `wallets`
* [x] `wallet_purchases`
* [x] `wallet_transactions`（監査用台帳：MVPでも推奨）
* [x] `drink_items`
* [x] `drink_orders`
* [x] `store_ledger_entries`（店舗売上台帳：MVPでも推奨）
* [x] `stripe_webhook_events`
...
### 4.2. 代表テーブル定義例（主キー/外部キー/制約）

#### 4.2.1 users

**目的**：認証（Devise）＋ロールを一元管理（Phase1は単一 `users` で統一）
...
* display_name（任意：表示名）
* email（Devise: database_authenticatable / validatable）
* encrypted_password（Devise: database_authenticatable）
...
* role は必須
* email は必須（※運用方針により nullable を許容するなら、別途 `user_authentications` を導入する）
* email はユニーク
