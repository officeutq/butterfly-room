...
- キャスト所属は店舗→招待→本人承認（ワンタイム・1週間）を必須とする
- 退会時はロール別クリーンアップとして所属解除を行う
- 退会を伴わないキャスト側からの任意所属解除は実装しない
...
4. キャスト所属は「店舗→招待→本人承認（ワンタイム・1週間）」を必須とする
5. 退会時のロール別クリーンアップを除き、キャスト側からの任意所属解除は実装しない
...
ユーザー自身の退会時は例外として、castの全所属解除、または唯一の有効なstore_adminが管理する店舗のクリーンアップを行う。

### 2.3 操作権限マトリクス（概要）

現行実装に基づく画面単位の詳細は、[11_画面アクセス権限マトリクス.md](11_画面アクセス権限マトリクス.md) を参照する。
...
* system_admin以外は確認モーダルと現在のパスワード確認を経て、自身の`users.deleted_at`を設定できる。
...
* `email`（`deleted_at IS NULL`のユーザー間でユニーク）
* `encrypted_password`
* `role`（integer / enum）
* `display_name`（任意）
* `phone_number`（`deleted_at IS NULL`のユーザー間でユニーク）
* `deleted_at`（停止日時 / null=有効）

**運用ルール**

* `deleted_at` が入ったユーザーは **ログイン不可**（Deviseの `active_for_authentication?` で拒否）
* メールログイン、電話番号ログイン、パスワード再設定、登録時の重複判定は`deleted_at IS NULL`のユーザーだけを対象とする
* 退会済みユーザーと同じメールアドレス・電話番号で再登録した場合は、別の`users.id`として扱う
...
* users：`deleted_at`（停止・自己退会 / ログイン不可、復活機能はPhase1では提供しない）
...
### 6.1 店舗：ブース情報／新規作成／閉鎖

#1291〜#1293で管理操作をブース情報へ統合し、新規作成は独立カードへ分離した。選択は[共通契約](design/current_selection.md)、画面・実機・環境ごとの確認範囲は[統合検証記録](design/booth_management_validation.md)を参照する。
...
#### 入出力（概要）
- 入力：ブース名、説明（任意）
- 出力：作成後はダッシュボード、管理操作後は同じブース情報

#### 仕様
- 管理対象の一覧は共通選択モーダルに集約する。有効選択の手動変更はヘッダーから行う。旧管理一覧GETは認可後ダッシュボードへ戻す。公開一覧・配信履歴は維持する。
- 店舗管理者は管理店舗、システム管理者は全店舗の閉鎖済みも選べる。一般キャストの候補は未閉鎖の担当ブースのみ。0／1／複数件と本人配信中・離席中の固定は共通選択に従う。
- 「ブース新規作成」は店舗管理者以上に表示する。店舗未選択なら共通店舗モーダルを使う。作成により `booths` レコードを生成し、既存の有効なブース選択は上書きしない。
- 未閉鎖のブース情報には共有・編集・履歴を表示し、管理権限と状態に応じて強制終了・閉鎖を追加する。確認した配信ID・世代と現在の選択・権限を実行時にも照合する。
- 強制終了は配信中・離席中が対象。閉鎖はオフライン・配信なし、または正常な準備を終了できる場合のみ許可する。配信中の閉鎖で強制終了を兼ねない。
- 閉鎖は `booths.archived_at = Time.current` を設定し、同じ選択と情報画面を維持する。閉鎖済みは履歴のみ操作でき、共有・編集・準備・開始は直接要求も拒否する。顧客向けの公開導線から除外する。
- 終了・未消化返却とAWS切断の成否は区別する。対象ブースの保存済み切断結果を表示し、専用の再確認ボタンは置かない。切断未確認でも閉鎖・履歴確認は可能とする。
- 一般キャストは認可された担当の閉鎖済み情報・履歴を直接閲覧できるが、閲覧対象を選択状態へ保存しない。管理操作・切断状態は表示しない。
...
#### 仕様（Phase1）
- 店舗管理者が招待を発行できる
- 招待はワンタイムで、1週間で失効する（MUST）
- キャスト本人がログイン状態で招待URLにアクセスし、明示的に承認した場合のみ所属成立
- キャスト→店舗の所属申請は提供しない

#### 例外と扱い
- 誤招待／誤承認が発生しても、ブースに紐づけない限り配信・売上に影響はない
- 退会を伴わないキャスト側の任意所属解除はPhase1では提供しない（Phase2以降）
- ユーザー自身の退会時はロール別クリーンアップとして所属を解除する
...
#1280の共通有効化後は、現在の配信中・離席中セッションの実配信者本人だけが消化できる。管理者の代理消化は許可しない。Serviceでブース→セッション→注文をロックして終了／返却との競合を順序付け、本人・現在参照・未終了・未閉鎖・配信権限を再確認する。消化通知は同じトランザクション内で保存し、最外側commit後に送信する。送信の再試行は同じコメントIDの表示通知だけを扱い、消化・売上・返却をやり直さない。
...
`CastMetricsQuery` は#1280の共通有効化後に次の実配信者別集計を使用する。有効化前だけ既存の準備作成者による集計を維持する。drink_ordersに人物列を追加して個人帰属を分散させない。
...
## 10. Phase2拡張：未ログイン公開導線

ログイン済みの公開ブースカードは、選択外なら視聴へ進み選択を維持する。選択中は担当キャストなら準備／復帰、管理者なら「視聴／配信」の選択を残す。非公開店舗の準備・開始禁止と公開状態の変更条件は[選択の共通契約7.1節](design/current_selection.md)・[実配信者設計1.1節](design/actual_publisher.md)に従う。

Phase1の認可・売上・配信状態の不変条件を維持したまま、閲覧専用の公開範囲を次のとおり拡張する。
...
### キャスト招待の状態・表示（#1237〜#1239）

`StoreCastInvitation.cancelled_at` がある招待は使用不可とし、承認処理でも拒否する。取消は物理削除せず、承認・共有完了との競合を行ロックで制御する。`shared_at` は共有・コピー操作の完了記録であり、相手への到達・承認を意味しない。共有済み・使用済みの招待は今回のモーダルでは取り消さない。`note` は既存値を含めて管理者専用とし、公開招待ページから除外する。

新しい招待一覧は取消済みを除外し、有効・使用済み・期限切れを新しい順に表示する。所属成立・ブース生成の既存仕様は変更しない。

### 店舗所属者情報・管理者招待（#1359）

所属一覧は `StoreMembership` の選択店舗のcast・adminを取得し、管理者のカードにはキャスト専用の所属解除・ブース欄を表示しない。`/admin/casts` の既定は所属一覧、`tab=invitations` はキャスト招待、`tab=admin_invitations` は管理者招待とする。

`StoreAdminInvitation` に `note`・`cancelled_at`・`shared_at`・`request_key` を追加し、発行者と発行キーの組を一意にする。既存値はNULLを許可し、過去の共有実績は補完しない。取消済みは `usable?` で拒否し、公開表示・登録・承認・既存所属者の自動使用にも適用する。共有・承認・取消を行ロックで順序付け、共有済み・使用済みの招待は取消不可とする。メモは管理者専用、共有完了は端末操作の成功記録であり、相手への到達・承認を意味しない。
