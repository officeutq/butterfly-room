...
#1226のUI調整では、初回AI入力・再検索・保存公開のボタンに既存の黄色の配色を使用する。「見つかった情報は、あとで確認・修正できます。この操作だけで店舗が公開されることはありません。」は通常の文字サイズと淡い背景で見やすくし、前半を太字にする。画面内の「戻る」リンクは設けず、共通ナビゲーションとブラウザの戻る操作は維持する。
...
- 画像あり・なしとも既存のJSON保存を利用し、保存失敗時に値、画像ファイル、バッジ、編集保護、情報源を同じ画面に残す。検索は再実行しない。保存確認、未保存変更警告、戻る、公開状態、運営専用項目を維持する。成功後は原則ダッシュボードへ戻る。#1234で追加した公開店舗詳細からの入口だけは、公開中なら同じ店舗の詳細へ戻し、非公開ならダッシュボードへ戻す。固定の `return_to=store_detail` を編集フォームから保存時にも送信し、保存失敗後の再試行でも戻り先を維持する。
