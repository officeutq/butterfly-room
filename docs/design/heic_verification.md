...
本書は**保存を伴わない検証実装**の記録。実機撮影画像・iPhone Safari・Android Chromeでの確認と本番採用判断は未完了である。本番のFilePond・Active Storage・`ImageAttachments::UpdateService` は変更しない。
...
| --- | --- |
| 入力ファイル | 空でない20MiB以下 |
| Workerの先頭画像 | 1600万画素・長辺8192px・縦横比8:1以下 |
