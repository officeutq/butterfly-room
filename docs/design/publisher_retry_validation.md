...
以下の全組合せの確認は未完了。2026-09-18、利用者からステージングでの配信開始・通常終了・強制終了が問題なく、操作に伴うエラーログもないと報告を受けた。追加回答は「PC・店舗管理者で配信・システム管理者で強制終了」。店舗管理者によるPCの開始・通常終了と、システム管理者による強制終了を正常系の確認済みとして記録する。ブラウザー名は未申告。[報告とログ照合の記録](../ops/publisher_retry_staging_verification.md#利用者の確認結果と残項目)を参照する。今回のスマートフォン実機、キャスト／システム管理者自身による配信操作は未確認として残す。以前の#1277等の実機結果を今回変更部分の確認済みとして転用しない。
