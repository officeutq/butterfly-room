...
以下の全組合せの確認は未完了。2026-09-18、利用者からステージングでの配信開始・通常終了・強制終了が問題なく、操作に伴うエラーログもないと報告を受けた。[報告とログ照合の記録](../ops/publisher_retry_staging_verification.md#利用者の確認結果と残項目)を参照する。役割・端末・ブラウザーの組合せは今回未申告のため、PC／スマートフォンそれぞれでキャスト・店舗管理者・システム管理者の全区分を確認済みにはしない。以前の#1277等の実機結果を今回変更部分の確認済みとして転用しない。
