...
### 3.3 開始確認失敗後の有限な回復（2026-09-17 / #1338）

開始確認の0.5秒・1秒・2秒の追加3回を維持する。上限後は同じ要求UUIDのDB状態を取得し、開始確定済みなら稼働中の同じSDKを維持する。未確定なら送信を止め、その要求だけを取消する。状態取得・取消の通信失敗も各段階で同じ間隔の追加最大3回とし、取消後の切断待ちはDB状態だけを有限回確認する。各HTTP要求は15秒で中断する。同じ操作の回復Promiseを共用し、同じ画面の再呼出で回数をリセットしない。

最終的に結果不明・切断未確認なら開始を許可せず、画面更新または最終切断失敗時の運用問い合わせを案内する。取消完了なら同じ準備の通常開始へ戻る。本人の再接続失敗では人物・初回時刻・live/awayを保持する。離脱後も元の要求の後処理は行うが、古いSDKで開始確定・送信再開せず、新画面の状態を上書きしない。

開始確認が追加3回後も失敗した場合、認可済み `POST /cast/stream_sessions/:id/publisher_confirmation_failure` へ要求UUIDと世代だけを送る。本人の現在の未確定要求だけを受け付け、`confirmation_failure_reported_at` により同じ要求のログを一度に限定する。成功済み・取消済み・他人・古い世代は記録しない。受信文字列をそのままログへ送らず、固定例外クラス `ReportPublisherConfirmationFailureService::RetryExhausted` と保存済み関連IDを記録する。通信完全断時のログ到達は保証しない。切断失敗のログとは別事象とする。
...
取消は切断意図を保存し、保存したIDの切断成功後に開始権を解放する。同じ準備とXは保持する。失敗時はcancel_pendingを表示し、D03の上限付きジョブで再試行する。未参加でも切断できるため、トークン期限まで一律に待たない。

本人Yの再接続は初回時刻・人物・セッションを維持する。以前のIDを切断して、新世代・新UUID・新参加者IDを発行する。切断意図を先に確定し、旧行の切断結果を保存してから、新行を別トランザクションで発行する。発行直前に権限・公開状態・対象・世代・未解放の開始権を再検証する。切断の失敗回数も成功も、後続の発行失敗で巻き戻さない。固定90秒等は置かない。別人はこの操作でYを引き継がない。
...
| 503 `publisher_state_unavailable` | 「配信状態を確認できません。再確認してください」。有限回の自動確認後、確認不能ならエラーを表示する |
...
2. 保存した参加者IDへの切断を最外側commit後に登録する。DB処理のrollback時にはAWSへ切断要求を送らない。
3. 同じDBトランザクションでセッションended・ended_at、対象boothのoffline・現在参照解除、未消化返却を確定する。Y・初回時刻・準備作成者は維持する。DB保存失敗なら返却もロールバックし、同じ要求から再試行する。
4. 最外側のDB確定後に保存したIDへ初回切断を行い、成功したIDだけdisconnected_at・released_atを保存する。失敗も回数と理由を保持し、終了通知・残高通知・必要な再試行ジョブを送る。通知失敗によって返却を再実行しない。既に終了済みの同じ対象への再試行は既存の結果と切断待ちを返す。
5. 切断待ちは再試行中と最終失敗を区別する。最終失敗時は「配信の終了と未消化ドリンクの返却は完了しました。配信接続の切断を確認できませんでした。」と案内する。閉鎖・所属解除・退会はDB成功なら続行する。

未配信準備で参加者IDがなく、外部の矛盾がない場合は接続記録を要求せず終了できる。旧準備を新列空欄だけで閉じ込めない。未知の旧接続が見つかった場合は移行前提の例外として対象を記録して判断を戻し、全Stageを削除しない。

### 5.3 終了画面・通知（2026-09-17 / #1339）

通常終了はブラウザーの映像・音声を止め、同じセッション・要求UUID・世代の終了を送る。HTTP応答が不明なら0.5秒・1秒・2秒で追加3回だけ自動確認し、終了・返却の確定が分かった場合はAWS切断が未確認でもリザルトへ進む。古い要求や権限喪失は無条件に再送しない。最終的に終了結果そのものが不明なら成功案内せず、再読込を案内して新規開始を止める。同じ画面の操作で上限をリセットしない。

強制終了の通知は終了・返却のDB確定後、AWS切断の成否にかかわらず送る。`publisher-ended` は配信画面のセッションIDと通知IDを照合し、同じSDKを退出・映像音声を停止してリザルトへ進む。別セッションの通知では停止・遷移しない。

リザルト・ブース情報は切断再試行中／最終失敗を分けて表示する。#1291〜#1293で旧管理一覧を廃止し、管理操作と対象ブースだけの切断結果をブース情報へ統合した。初期HTML・通知・状態GETは同じブース範囲を使い、閲覧者が別ブースに持つ未確認接続を混在させない。閉鎖済みの個別操作は履歴のみで、管理権限があれば状態・エラーも表示する。確認範囲は[管理統合の検証記録](booth_management_validation.md)を参照。DBの切断成功・最終失敗後に私用のTurboストリームで該当セッション／ブースの表示を更新する。通知開始前の更新や通知未達に備え、表示初回＋0.5秒・1秒・2秒の有限なGET確認も行う。通知・取得とも失敗した場合は再試行中と断定し続けず、結果未確認と再読込の案内へ切り替える。取得はAWSを呼ばず、一般公開詳細へ内部エラーを流さない。

配信・準備・リザルト・旧管理画面の専用「再確認」「切断再試行」と旧POSTルートを撤去する。通常の開始・開始取消・終了は維持し、本人の再接続取消後の切断未確認だけでは通常終了を妨げない。継続中の定期監視や一定時間の通信断による強制停止は追加しない。AWS切断と終了通知がともに機能しない障害時の即時停止は保証しない。
...
AWS SDKの内蔵再試行は `retry_limit: 0, max_attempts: 1` で無効化する。`ivs_disconnect_limits` のアカウント・リージョン別行を共用し、同じDBのapp／worker全体で0.25秒以上の間隔を空ける。空き枠待ちはAPI試行回数を消費しない。別DBの環境や運用操作とは調整できないため、AWS側の毎秒5件の枠で拒否された場合も通常の失敗として有限回扱う。AWS障害や過剰な外部呼出をアプリ内の制限だけで解消する保証はない。
