...
リザルト・旧管理画面は切断再試行中／最終失敗を分けて表示する。DBの切断成功・最終失敗後に私用のTurboストリームで該当セッション／ブースの表示を更新する。通知開始前の更新や通知未達に備え、表示初回＋0.5秒・1秒・2秒の有限なGET確認も行う。通知・取得とも失敗した場合は再試行中と断定し続けず、結果未確認と再読込の案内へ切り替える。取得はAWSを呼ばず、一般公開詳細へ内部エラーを流さない。
