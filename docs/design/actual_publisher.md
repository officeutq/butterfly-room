...
2026-09-15：#1282〜#1287・#1289の実装は統合済み。#1288の[全機能連携の確認記録](actual_publisher_validation.md)と[有効化・復旧手順](../ops/actual_publisher_rollout.md)で、実装済み・実環境で未適用の範囲を分けて管理する。旧履歴は6.4節のX補完に統一する。
...
#1287はDBの読み取り調査結果とX補完への移行手順を整理する。保存済み消化通知・通常コメント・操作履歴の人物は変更しない。実配信者を立証した結果として補完値を扱わない。調査済みの範囲と実環境への未適用は [旧履歴の調査結果](legacy_publisher_evidence.md) に記録し、具体的な補完一覧・差分の適用確認は #1288 に引き継ぐ。
...
2026-09-14の#1303実IVS確認では、旧準備→Y配信成功→離席→Y再接続→終了を同じtestセッションで行い、サーバーの終了処理によるSDKのStage退出、DBのended/offline/現在参照解除、Y・X・初回時刻の保持、同じ終了要求への同一結果を確認した。今回の終了と再送の応答は合計約0.35秒であり保証値ではない。一時Stageを削除してGetStageの404を確認し、testデータも削除済み。切断失敗・DB失敗・通信応答消失・退会競合は別の代替応答／DB並行テストで確認する。本番権限・全体有効化は未実施。
