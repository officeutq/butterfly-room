...
新方式を指定した読み取り専用の`CastMetricsQuery`で、全店舗の消化台帳合計との一致も確認した。Xに補完された行は25,301pt・14,953,270秒、対象外の不明行は611,400pt、全体636,701pt。ステージング・本番は引き続き未適用。ローカルの補完は新方式の画面を有効化する設定変更ではない。
