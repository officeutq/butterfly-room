...
テストDBへの適用・復旧と、実環境への適用を区別する。上記事前確認時点ではステージング・本番は未調査・未適用。その後の2026-09-14のDB読み取り結果は [旧履歴の調査結果](legacy_publisher_evidence.md) を参照する。読み取り調査時点では3環境とも未補完で、9月15日に#1288で実施した適用結果は以下に記録する。

### 2026-09-15 ローカルへの適用

09:49 JSTの直前調査はセッション734件・配信中0・消化台帳636,701pt・消化通知27件。ユーザーのX補完への統一決定に従い、上記と同じ固定一覧・SHAを使い、コード`1e51a675305fc36560a111af9f65149a41cd661f`でローカル開発DBへ適用した。結果は62件applied、除外・失敗なし。新しい対象を検索して一覧に加えていない。

09:51 JSTの再調査で対象62件のY=X・source・記録時刻を確認。元のX・状態・開始終了時刻・更新時刻・現在参照・各配信の台帳件数／ポイント／時刻・保存済み消化通知の人物と時刻は不変。対象外672件は不変だった。実行記録は`tmp/issue1288-local-apply.jsonl`、前後の読み取り結果は`tmp/issue1288-local-before.json`・`tmp/issue1288-local-after.json`に保持する。

新方式を指定した読み取り専用の`CastMetricsQuery`で、全店舗の消化台帳合計との一致も確認した。Xに補完された行は25,301pt・14,953,270秒、対象外の不明行は611,400pt、全体636,701pt。このローカル適用時点ではステージング・本番は未適用。ローカルの補完は新方式の画面を有効化する設定変更ではない。

### 2026-09-15 stagingへの適用

コード`6a23393137d938f04a9a2f52ffa1a2d0d3b8b78f`で、配信中0とDBバックアップを確認し、新列導入後に固定一覧を作成・確認して12件をXへ補完した。未開始の準備2件を除外。元の14行のX・状態・時刻・現在参照・台帳・保存済み通知は不変。個人実績の合計は0pt・18秒。

固定一覧のSHA-256は`2e680cad391341eaddd6371fbd25f6ef492975abc109782f412e4dc3c2175716`、基準時刻は2026-09-15 01:09:35.886426 UTC。適用件数12、競合・失敗なし。保管場所・バックアップ・有効化結果は[移行手順](../ops/actual_publisher_rollout.md)を参照。

## 2026-09-15 本番適用

DB単体のバックアップと直前のlive/away=0を確認し、新列導入後に作成した固定一覧へ適用した。対象はID 5の1件で、作成者X=14へ補完。未開始準備6件は`not_ended`で除外し、元の7行のX・時刻・状態・現在参照・台帳・保存済みコメントが不変であることを比較した。補完後の個人集計は0pt・158秒。

固定一覧のSHA-256は`dd40b8e4d2540d87c62f65699299dec4a20b6e1200b98371a976bb953e775a7f`、基準時刻は2026-09-15 02:45:40.738621 UTC。適用件数1、競合・失敗なし。app・workerの新方式有効化も確認済み。バックアップ・固定一覧・前後比較の保管先と確認範囲は[移行手順](../ops/actual_publisher_rollout.md)を参照する。本番DBへのrestoreは実施していない。
