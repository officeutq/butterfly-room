...
- 2026-09-10（#1241）：正本に「キャストについて」を追加し、11分類・53問へ同期。「利用開始について」の次に、キャストの説明、招待の仕組み、招待手順、所属・招待状況の確認方法を新設し、複数店舗への所属と退店時の所属解除の2問を移動した。
- 同更新で、招待承認後のブース作成、初回店舗設定での公開、店舗ページ遷移後のチュートリアル開始、店舗管理者の配信開始経路を現行の画面に合わせて補足した。
