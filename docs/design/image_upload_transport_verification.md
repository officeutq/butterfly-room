...
検証後の初期方式は**Rails multipart**とする。iPhone 15 Proの同一約3.96MB画像ではdirectのほうが速かったが、当面の同時接続数、実装・運用・失敗状態の単純さを優先した。source 20MiB、display 5MiB、全体26MiB、クライアント待ち45秒を本番実装の境界とし、ステージングで容量・低速条件を再確認する。保存Serviceは送信方式へ依存させず、利用増加時にdirectへ切り替えられる構成を維持する。最終仕様は [Cropper.js画像アップロード確定設計](image_upload_cropper_architecture.md) を正とする。
...
JSONと画像の**寸法・形式・範囲**の整合を検査するが、表示用画像の画素が編集元の指定範囲から生成されたことまではサーバーで再描画して証明していない。本番では保存Serviceが実際の編集元Blob IDをcrop dataへ設定し、source実寸法・用途・出力・範囲・比率を再検査する。詳細は確定設計5節を参照する。
...
- Rails側の検証用JPEG容量検査は、WebサーバーやRackによるmultipart受信前の全体容量制限ではない。本番契約はmultipart全体26MiB、クライアント待ち45秒とし、body解析前の入口制限を#1149で実装する。未知長body、低速接続、S3処理、ピークメモリは#1153のステージング境界確認をrelease gateとする。
...
7. 検証結果は#1132へ記録済み。#1134でRails multipartと本番容量候補を確定し、通常フォームの実機・低速・境界確認は#1153へ引き継いだ。
