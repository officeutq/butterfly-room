...
2026-09-15更新。**ユーザー判断により、旧履歴は作成者Xの実績として補完する方針に統一する。** IVSの過去履歴による人物調査・証拠補正・そのための権限追加は不要となった。DB読み取り調査と移行手順を整理後、#1288でローカル62件・staging12件・本番1件の固定一覧へX補完を適用し、staging・本番の新方式有効化も完了した（[適用記録](legacy_publisher_backfill.md)）。以下の調査表は9月14日時点の状態として保持する。
...
3. 適用前後の件数・未適用・再開／復旧方法を環境別に記録する。この読み取り調査時点では3環境とも未補完であり、最新状態を適用直前に再確認する。
4. 全体検証と共通有効化を行う。関連の店舗／ブース選択・招待Epicが未実装の場合は、その連携確認を未完了として残す。親 #1280 を先に閉じない。

## 2026-09-15の後続適用

上記は読み取り調査時点の記録。その後、#1288でローカル62件・staging12件・本番1件の固定一覧へX補完を適用した。staging・本番は新列導入とapp／workerの共通有効化も実施済み。本番の旧準備6件と元の履歴・金額も保持した。今後の配信制御に必要な実行ロールの3権限はユーザーが追加し、実接続で利用を確認。過去IVS調査の追加権限は不要のまま。[適用結果](../ops/actual_publisher_rollout.md)と[全機能連携確認](actual_publisher_validation.md)で最新状態を管理する。
