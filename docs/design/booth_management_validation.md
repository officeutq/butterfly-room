...
追加報告により、スマートフォンのシステム管理者でも4項目すべて正常だったことを確認した。この管理操作に使ったスマートフォンの機種・ブラウザー名は未申告であり、先の配信側のiPhone 15 Proと同一とは推測しない。

## 残る実機確認

管理画面のPC・スマートフォン操作はシステム管理者で確認できた。残る実機確認は店舗管理者での新規作成→ダッシュボード、ブース情報から強制終了、閉鎖、閉鎖後の履歴・ヘッダー再選択の4項目。端末はPC・スマートフォンのどちらでもよく、管理店舗の権限で新しい導線を操作できることを補う。システム管理者の結果を店舗管理者の実機確認済みとして扱わない。
