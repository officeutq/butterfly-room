...
画像を明示的に削除した場合は、保存前でも次のAI検索で取得できる。取得失敗・中断では削除予定を維持し、成功した差し替えも最後の保存で確定する。
...
| 再検索 | 設定済み・手動選択済み・仮反映済み画像は保護。「画像を削除」後は保存前でも次のAI検索で取得可能。変更取消だけでは再取得しない |
| 削除後の取得 | 取得失敗・中断では削除予定を保持。成功時は差し替え予定とし、元画像の競合検知と最後の保存で確定する方式を維持 |
