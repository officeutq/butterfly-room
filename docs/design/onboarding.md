...
表示と進捗更新は、対象店舗の管理者所属を持つ店舗管理者、およびシステム管理者に限定する。退会済み利用者には許可しない。共通レイアウトの表示と、ダッシュボード到達・スキップによる更新は `Stores::AdvanceOnboarding.allowed?` で同じ権限を確認する。

招待を承認したキャストに店舗・ブースが選択されても、店舗管理者向けの案内は表示しない。キャストによるプロフィール・ブース編集、ホーム・ダッシュボードの閲覧で店舗の進捗を進めない。管理者がログアウトして同じブラウザーからキャストとしてログイン・新規登録した場合も同様とする。進捗は店舗に残し、権限のある管理者が続きから再開できる。
...
| `setup_drinks` | 対象店舗を管理できる利用者のダッシュボード到達で更新し、「店舗情報」→「ドリンクメニューを編集」→追加・編集フォームを案内 |
...
`setup_drinks` はダッシュボードの `store-information-card`、管理用店舗情報の `store-drinks-edit`、ドリンク編集の既存フォームを画面に応じて案内する。情報・メニューの閲覧だけでは完了にせず、保存成功後に店舗情報へ戻った時は完了済みになる。既存店舗の進捗はリセットせず、同じ権限確認とスキップ・復元処理を利用する（#1253）。
...
- `app/controllers/admin/onboardings_controller.rb`
