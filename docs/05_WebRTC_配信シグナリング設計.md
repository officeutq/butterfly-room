...
ブース・店舗の選択は#1255の[共通契約](design/current_selection.md)へ改修予定（#1271は設計のみ）。選択側は#1280で実装済みのDB参照を利用し、準備作成者・未確定の開始権を本人配信と読み替えない。選択だけで準備・配信開始・終了を行わず、明示的な準備画面への移動で既存Serviceを呼ぶ。他者配信中や閉鎖済みは選択できても準備・開始できない。新方式の配信成功確定・世代・切断再試行の契約は維持する。
