...
ブース・店舗の選択は#1255の[共通契約](design/current_selection.md)に従い、#1274〜#1276で実装済み（環境反映・実機確認は[ステージング記録](ops/current_selection_staging_verification.md)を参照）。選択側は#1280で実装済みのDB参照を利用し、準備作成者・未確定の開始権を本人配信と読み替えない。選択だけで準備・配信開始・終了を行わず、明示的な準備画面への移動で既存Serviceを呼ぶ。他者配信中や閉鎖済みは選択できても準備・開始できない。新方式の配信成功確定・世代・切断再試行の契約は維持する。

#1291〜#1293で管理操作の入口をブース情報へ統合した。強制終了・閉鎖は既存Serviceを呼び、成功後は同じ情報へ戻る。初期表示・通知・`publisher_disconnect_state?scope=booth`は対象ブースだけの保存済み切断結果を使う。閲覧・状態取得で切断要求や試行回数を変更せず、有限な取得後も結果が不明なら再読込を案内する。閉鎖済みは履歴と状態表示のみで、復旧操作ボタンは置かない。[統合検証記録](design/booth_management_validation.md)を参照。
...
- **配信開始 ⇔ 配信終了**：IVS publish の開始/停止。終了結果を確認して配信リザルトへ進む。新経路では外部切断待ちでもDB終了・返却が確定すればリザルトと切断結果を案内する
...
- 終了・返却はIVS切断失敗時もDBへ確定し、外部切断だけを記録して再試行する。閉鎖・退会後も記録を保持する。同じブース・人物の次回開始は保存済み接続の切断成功を確認してから許可する。
