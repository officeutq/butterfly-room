...
export default class extends Controller {
  ...
  static values = { createUrl: String, storeId: Number, requestKey: String, admin: Boolean }
  ...
  async saveNote() {
    ...
    await this.request(this.invitation.update_url, "PATCH", { [this.adminValue ? "store_admin_invitation" : "store_cast_invitation"]: { note } })
    this.savedNote = note
  }
  ...
  async share() {
    ...
    try {
      ...
      this.completed = true
      this.updateCompleted()
      const guiding = !this.adminValue && ["invite_cast", "create_invite", "go_dashboard_for_drinks"].includes(this.invitation.step)
      this.statusTarget.textContent = mode === "copy"
        ? `招待URLをコピーしました。LINEなどに貼り付けて、${this.adminValue ? "店舗管理者" : "キャスト"}本人に送ってください。` + (guiding ? "送ったら『閉じる』を押して、次の設定に進みましょう。" : "")
        : guiding
  ...
  async close() {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("invitation:changed", { detail: { storeId: this.storeIdValue } }))
      this.allowHide = true
  ...
  updateCompleted() {
    ...
    if (!this.adminValue) window.dispatchEvent(new CustomEvent("cast-invitation:updated"))
  }
