...
class Admin::StoreEditReturnTest < ActionDispatch::IntegrationTest
  ...
  test "legacy detail entry carries its return destination through the form without switching the selected store" do
    ...
    assert_select "a.store-edit__back[href=?]", admin_store_path(@store), count: 1
    patch admin_store_path(@store), params: { return_to: "store_detail", store: { description: "更新後" } }, as: :json
    assert_response :success
    assert_equal admin_store_path(@store), response.parsed_body["redirect_url"]
  ...
  [ :html, :json ].each do |format|
    test "#{format} detail save returns to the saved store" do
      ...
      assert_saved_return(format, admin_store_path(@store))
      assert_equal "変更済み", @store.reload.description
    end

    test "#{format} unpublishing through detail returns to management information" do
    ...
    test "#{format} saves from existing entries keep the management information destination" do
  ...
  test "unpublished stores use the management information back link even with a legacy detail entry marker" do
  ...
  test "unrecognized or structured return destinations are ignored on edit and save" do
    [ nil, "unknown", "https://example.com/", "//example.com/", admin_store_path(@store),
      ...
      assert_saved_return(:json, admin_store_path(@store))
    end
  end

  test "failed JSON save leaves the store unchanged and retry can still return to management information" do
  ...
  test "failed HTML save preserves input and drops the obsolete detail marker" do
    save_store(:html, return_to: "store_detail", store: { name: "" })

    assert_response :unprocessable_entity
    assert_select "input[name='store[name]'][value='']"
    assert_select "a.store-edit__back[href=?]", admin_store_path(@store), count: 1
    assert_select "input[name=return_to]", count: 0
  end

  test "legacy detail entry never grants edit rights to another store" do
