...
class Cast::PublisherPreparationTest < ActionDispatch::IntegrationTest
  ...
  test "S06 公開入口 選択入口 配信画面直リンクの照会失敗で元の選択を維持する" do
    new_control do
      ...
      assert_select ".cast-live-screen", count: 0
      [ -> { post enter_as_cast_booth_path(@booth) }, -> { get live_cast_booth_path(@booth) } ].each do |request_action|
        request_action.call
        assert_response :conflict
        assert_equal @selected_booth.id, session[:current_booth_id]
      end
      post cast_current_booth_path, params: { booth_id: @booth.id, return_to_key: "booth_show" }
      @client.stub_responses(:get_stage, { stage: { arn: @booth.ivs_stage_arn } })
      post enter_as_cast_booth_path(@booth)
  ...
  test "S06 モーダルの候補GETは外部照会をしない" do
  ...
  test "未選択のURL対象への準備POSTを拒否して元の選択を維持する" do
    new_control do
      select_previous_booth
