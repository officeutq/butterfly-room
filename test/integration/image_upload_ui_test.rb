...
class ImageUploadUiTest < ActionDispatch::IntegrationTest
  test "store thumbnail uses the social Cropper editor without FilePond" do
    ...
    assert_select "input[type='file'][name='store[thumbnail]']", count: 0
  ...
  test "store Cropper editor exposes a complete current pair for re-editing" do
    store_admin = create_user("image-ui-current-store-admin", :store_admin)
    store = Store.create!(name: "現在画像店舗")
    StoreMembership.create!(store:, user: store_admin, membership_role: :admin)
    attach_fixture(store.thumbnail_source, "store-source.jpg")
    attach_fixture(store.thumbnail, "store-display.jpg")
    store.update!(thumbnail_crop_data: {
      "schemaVersion" => 1,
      "ratioKey" => "social",
      "sourceBlobId" => store.thumbnail_source.blob.id,
    ...
    assert_response :success
    assert_select "#image-attachment-editor-image-pair" do
      ...
      assert_select(
        "input[name='image_pair[expected][display_blob_id]'][value='#{store.thumbnail.blob.id}']",
        count: 1
      )
    end
  ...
  test "cast booth thumbnail uses the social Cropper editor without FilePond" do
    ...
    assert_response :success
    assert_booth_cropper_form
  end

  test "admin booth creation uses the same social Cropper editor" do
    store_admin = create_user("image-ui-admin-booth", :store_admin)
    store = Store.create!(name: "画像UI管理ブース店舗")
    StoreMembership.create!(store:, user: store_admin, membership_role: :admin)
  ...
  test "booth Cropper editor exposes a complete current pair for re-editing" do
    cast = create_user("image-ui-current-booth-cast", :cast)
    store = Store.create!(name: "現在画像ブース店舗")
    booth = Booth.create!(store:, name: "現在画像ブース")
    BoothCast.create!(booth:, cast_user: cast)
    attach_fixture(booth.thumbnail_image_source, "booth-source.jpg")
    attach_fixture(booth.thumbnail_image, "booth-display.jpg")
    ...
    assert_select "#image-attachment-editor-image-pair" do
      ...
      assert_select "[data-image-attachment-editor-current-crop-data-value]", count: 1
      assert_select(
        "[data-image-attachment-editor-current-source-blob-id-value='#{booth.thumbnail_image_source.blob.id}']",
        count: 1
      )
    end
    assert_includes response.body, "/rails/active_storage/blobs/proxy/"
  ...
  test "legacy booth image remains visible and deletable but is not re-editable" do
    cast = create_user("image-ui-legacy-booth-cast", :cast)
    store = Store.create!(name: "旧画像ブース店舗")
    booth = Booth.create!(store:, name: "旧画像ブース")
    BoothCast.create!(booth:, cast_user: cast)
    attach_fixture(booth.thumbnail_image, "legacy-booth.jpg")
    ...
    assert_select "#image-attachment-editor-image-pair" do
      ...
      assert_select(
        "input[name='image_pair[expected][display_blob_id]'][value='#{booth.thumbnail_image.blob.id}']",
        count: 1
      )
    end
  end
  ...
  def assert_booth_cropper_form
    ...
    assert_select "input[type='file'][name='booth[thumbnail_image]']", count: 0
